...
def count_unique(string1):
    ...
    uncounted_words = string1.split(" ")
    for each_word in uncounted_words:
        if each_word in words_and_counts:
            words_and_counts[each_word] = words_and_counts[each_word] + 1
        else:
            words_and_counts[each_word] = 1
...
def unique_common_items(list1, list2):
    ...
    # We think dictionaries have to have unique keys, so we want to use this 
    # data structure to check for uniqueness. We're not sure if this is enforced by 
    # Python, i.e., will the compiler throw an error or not if the keys are NOT unique.
    # Check on Monday!
    for each_item in list_of_common_items:
        unique_dict[each_item] = "dummy"
